document.addEventListener('DOMContentLoaded', async () => {
            ...
            const imageCarouselHTML = producto.imagenes && producto.imagenes.length > 0
                ...
                <div class="product-image-container">
                    ${producto.imagenes.map((img, index) => `
                        <img
                            ${index === 0 ? `src="https://cloudiarg.com/uploads/${img}"` : `data-src="https://cloudiarg.com/uploads/${img}"`}
        ...
        productsContainer.querySelectorAll('.delete-btn').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                if (confirm('¿Estás seguro de que quieres eliminar este producto?')) {
                    try {
                        const response = await fetch(`https://cloudiarg.com/api/productos/${productId}`, {
                            method: 'DELETE',
                            headers: {
                ...
                if (producto) {
                    ...
                    if (producto.imagenes && producto.imagenes.length > 0) {
                        producto.imagenes.forEach(imgName => {
                            const img = document.createElement('img');
                            img.src = `https://cloudiarg.com/uploads/${imgName}`;
    ...
    const fetchProducts = async () => {
        ...
        try {
            const response = await fetch('https://cloudiarg.com/api/productos', {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
